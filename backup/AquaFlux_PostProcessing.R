...
convert.sapflux.vol.units = function( sapflux.data, starting.time.units, disired.time.units ){
  # check for valid units
  ...
  if (starting.time.units==("cm3")){ starting.time.units="g"}
  if (disired.time.units==("cm3")){ disired.time.units="g"}
  ...
  if (starting.time.units=="g"){   x = x}
  if (starting.time.units=="m3"){ x = x*10^(+6) }
  if (starting.time.units=="L"){  x = x*1000 }
  if (starting.time.units=="gal"){ x = x*3785.41 }
  
  # convert standard units (s) to disired (disired.time.units)
  if (disired.time.units=="g"){   x = x}
  if (disired.time.units=="m3"){ x = x/10^(+6) }
  if (disired.time.units=="L"){  x = x/1000 }
  if (disired.time.units=="gal"){ x = x/3785.41 }
