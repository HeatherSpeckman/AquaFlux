...
.BrokenSensors.lowAverage = function(dT.local,MinDT){
  y.med = median(dT.local, na.rm=T)
  if (y.med< MinDT & is.na(y.med)==F ){  sensor.fail = T } else { sensor.fail=F}
  sensor.fail
}
...
.BrokenSensors.detect = function(v){
  ...
  for (tree.name in v$sapflux.names){
    ...
    fail.low = .BrokenSensors.lowAverage(g$dT.local,v$MinDT)
